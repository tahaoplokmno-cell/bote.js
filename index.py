...
REDEMPTION_INSTRUCTIONS = (
    "\n\n📦 **تعليمات الاستبدال:**\n"
    "1️⃣ ادخل إلى الموقع الرسمي للشحن (مثال: midasbuy.com حسب اللعبة)\n"
    "2️⃣ اختر لعبتك وأدخل آيدي حسابك\n"
    "3️⃣ اختر «استرداد الأكواد» وأدخل الكود أعلاه\n"
    "4️⃣ اضغط استرداد\n"
...
def get_admin_panel(db):
    """توليد لوحة تحكم ديناميكية بالإحصائيات"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in db.get("banned", {}):
        await update.message.reply_text("🚫 حسابك محظور من استخدام هذا البوت. تواصل مع الدعم إن كنت تعتقد أن هذا خطأ.")
        return
    ...
    name = escape_md(update.effective_user.first_name or "مستخدم")
    text = (
...
async def panel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = str(update.effective_user.id)
    if is_admin(db, user_id):
        admin_panel_markup = get_admin_panel(db)
        await update.message.reply_text("🛸 **لوحة التحكم الإدارية**", reply_markup=admin_panel_markup, parse_mode='Markdown')
    else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.get("bot_maintenance", False) and not is_admin(db, user_id):
        await update.message.reply_text("🛠️ البوت حالياً في وضع الصيانة، الرجاء المحاولة لاحقاً. نعتذر عن الإزعاج 🙏")
        return
    ...
    if ud.get('awaiting_charge_proof'):
        ...
        await context.bot.send_message(
            ...
            f"👤 {escape_md(update.effective_user.first_name or 'مستخدم')}\n🆔 {user_id}\n"
            f"💰 {amount} {'$' if currency=='usd' else 'ل.س'} = **${usd_amount:.2f}**\n"
            f"🧾 رقم/مرجع العملية: {escape_md(text)}",
            parse_mode='Markdown',
    ...
    if ud.get('awaiting_complaint'):
        ...
        try:
            ...
            await update.message.reply_text("✅ تم استلام رسالتك وسيتم الرد عليك بأقرب وقت ممكن 🙏")
            await notify_admin_dm(context, f"⚠️ تعذر إرسال الشكوى للقناة: {e}\nشكوى من {user_id}: {escape_md(text)}")
        return
    ...
    if ud.get('awaiting_add_balance'):
        try:
            ...
            log_activity(db, f"إضافة يدوية ${amount} لـ {target_id} من الأدمن")
            save_db(db)
            await update.message.reply_text(f"✅ تم إضافة ${amount} إلى {escape_md(db['users'][target_id]['name'])}")
            await context.bot.send_message(target_id, f"🎉 تم إضافة ${amount} إلى محفظتك!")
        except:
            await update.message.reply_text("❌ الصيغة غير صحيحة! استخدم: `آيدي|المبلغ` — حاول مجدداً ", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_sub_balance'):
        try:
            ...
            if amount <= 0 or target_id not in db["users"]:
                raise ValueError
            update_balance(db, target_id, -amount)
            log_activity(db, f"خصم يدوي ${amount} من {target_id} من الأدمن")
            save_db(db)
            await update.message.reply_text(f"✅ تم خصم ${amount} من {escape_md(db['users'][target_id]['name'])}")
    ...
    if ud.get('awaiting_ban_user'):
        ...
        if target_id not in db["users"]:
            await update.message.reply_text("❌ لا يوجد مستخدم بهذا المعرف. حاول مجدداً أو اضغط الزر أدناه للإلغاء.", reply_markup=CANCEL_BTN)
            return
        db.setdefault("banned", {})[target_id] = True
        log_activity(db, f"حظر المستخدم {target_id}")
        save_db(db)
    ...
    if ud.get('awaiting_search_bot_order'):
        ...
        if not found:
            await update.message.reply_text("❌ لم يتم إيجاد أي طلب بوت بهذا المعرف.")
            return
        for oid, o in found[:5]:
            price_txt = f"${o['price']:.2f}" if o.get('price') is not None else "غير محدد"
            msg = (
                f"🤖 **طلب بوت** `{oid}`\n👤 الزبون: `{o.get('user_id')}`\n💬 التواصل: {escape_md(o.get('contact',''))}\n"
                f"📝 الوصف: {escape_md(o.get('desc',''))}\n🖥️ السيرفر: {escape_md(o.get('srv_name',''))}\n💰 السعر: {price_txt}\n"
                f"📌 الحالة: {o.get('status')}\n🗒️ التفاصيل المحفوظة:\n{escape_md(o.get('details') or 'لا يوجد')}"
            )
            buttons = []
            if o.get('file_id'):
                buttons.append([InlineKeyboardButton("📂 إعادة إرسال الملف للزبون", callback_data=f"resend_botfile#{oid}")])
    ...
    if ud.get('awaiting_add_category'):
        try:
            ...
            await update.message.reply_text("❌ الصيغة غير صحيحة! استخدم:\n`parent_id_او_root|games_او_cards_او_numbers|الاسم`.", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_add_product'):
        try:
            ...
            await update.message.reply_text("❌ الصيغة غير صحيحة! استخدم:\n`parent_id|kind|الاسم|السعر`\nkind: game_code / card / whatsapp_number / telegram_number.", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_toggle'):
        ...
        if not node:
            await update.message.reply_text("❌ لا يوجد عنصر بهذا المعرف. حاول مجدداً أو اضغط الزر أدناه للإلغاء.", reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_charge'):
        try:
            ...
            await update.message.reply_text(
                ...
                f"أرسل الآن **صورة الوصل** أو **اكتب رقم/مرجع العملية** لتأكيد طلبك:", parse_mode='Markdown', reply_markup=CANCEL_BTN
            )
        except:
            await update.message.reply_text("❌ اكتب رقماً صحيحاً! ", reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_refund'):
        try:
            ...
            btn = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ تأكيد الطلب", callback_data="confirm_refund")],
                [InlineKeyboardButton("❌ إلغاء", callback_data="cancel_flow")]
            ])
            await update.message.reply_text(
                f"💰 المبلغ: {amount} {'$' if currency=='usd' else 'ل.س'} = **${usd_amount:.2f}**\n"
                f"هل تريد تأكيد طلب الاسترجاع؟", reply_markup=btn, parse_mode='Markdown'
            )
        except:
            await update.message.reply_text("❌ اكتب رقماً صحيحاً! ", reply_markup=CANCEL_BTN)
            return
    ...
    if ud.get('awaiting_game_id'):
        ...
        await update.message.reply_text(
            f"🎁 {escape_md(node['name'])}\n💰 السعر: ${node['price']}\n🆔 آيدي اللعبة: {escape_md(game_id)}\n\n"
            f"⚠️ تأكد من صحة الآيدي جيداً قبل التأكيد.\nهل تريد تأكيد الطلب؟",
    ...
    if ud.get('awaiting_phone_amount'):
        try:
            ...
            await update.message.reply_text(
                f"📱 **مراجعة طلب شحن الهاتف**\n📞 الرقم: {escape_md(ud.get('phone_number',''))}\n"
                f"📶 الشبكة: {ud.get('card_type')}\n💰 المبلغ: {amount:,.0f} ل.س (${usd_amount:.2f})\n\n"
                f"تأكد من صحة رقم الهاتف قبل التأكيد. هل تريد المتابعة؟",
                reply_markup=btn, parse_mode='Markdown'
            )
        except:
            await update.message.reply_text("❌ اكتب رقماً صحيحاً! ", reply_markup=CANCEL_BTN)
    ...
    if ud.get('awaiting_bot_price'):
        ...
        if not m:
            await update.message.reply_text("❌ لم أتمكن من إيجاد رقم بالسعر! اكتب مثلاً: `10$` أو `10` — حاول مجدداً.", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return
        ...
        await context.bot.send_message(target_id, f"💰 **السعر المتفق عليه لطلب البوت:** ${price:.2f}\nاضغط الزر أدناه للموافقة والدفع من محفظتك:", parse_mode='Markdown', reply_markup=pay_btn)
        await update.message.reply_text(f"✅ تم إرسال السعر (${price:.2f}) إلى {target_id} مع زر الدفع.")
        return
    ...
    if ud.get('awaiting_bot_notes'):
        ...
        save_db(db)
        await update.message.reply_text(f"✅ تم حفظ التفاصيل لطلب `{order_id}`. يمكنك استرجاعها لاحقاً من لوحة التحكم ← 🔎 بحث عن طلب بوت.", parse_mode='Markdown')
        return
    ...
    if ud.get('awaiting_delivery_code'):
        ...
        delivery_text = (
            f"✅ **تم تفعيل طلبك!**\n━━━━━━━━━━━━━━━━━━━━\n"
            f"🎁 المنتج: {escape_md(order.get('item_name', 'رقم/بطاقة'))}\n"
        ...
        try:
            await context.bot.send_message(target_id, delivery_text, parse_mode='Markdown')
            await update.message.reply_text(f"✅ تم تسليم الطلب (رقم {order_id}) للزبون بنجاح.")
            db['stats']['purchases'] += 1
            log_activity(db, f"تسليم طلب شراء #{order_id} لـ {target_id}")
    ...
    if ud.get('awaiting_bot_file'):
        ...
        await update.message.reply_text(f"✅ تم إرسال الملف (نص) إلى {target_id} وتم حفظه لطلب `{order_id}` لإعادة الإرسال لاحقاً.", parse_mode='Markdown')
        return
...
async def handle_photo_and_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get('awaiting_bot_file') and update.message.document:
        ...
        await update.message.reply_text(f"✅ تم إرسال الملف إلى {target_id} وتم حفظه لطلب `{order_id}` لإعادة الإرسال لاحقاً.", parse_mode='Markdown')
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("resend_botfile#"):
        ...
        if not order or not order.get('file_id'):
            await query.answer("❌ لا يوجد ملف محفوظ لهذا الطلب", show_alert=True)
            return
        try:
            await context.bot.send_document(order['user_id'], order['file_id'], caption="📂 تفضل ملف البوت الخاص بك مجدداً!")
            await query.answer("✅ تم إعادة الإرسال", show_alert=True)
        except Exception as e:
    ...
    if data == "open_panel":
        if not is_admin(db, user_id):
            ...
            return
        admin_panel_markup = get_admin_panel(db)
        await query.edit_message_text("🛸 **لوحة التحكم الإدارية**", reply_markup=admin_panel_markup, parse_mode='Markdown')
        return
    ...
    if data.startswith("adm#"):
        ...
        if action == "stats":
            ...
            await query.edit_message_text(
                ...
                f"📩 شكاوى/استفسارات: {s.get('complaints',0)}",
                parse_mode='Markdown')
            return
        ...
        if action == "pending_list":
            ...
            for oid, o in list(orders.items())[:25]:
                lines.append(f"`{oid}` — {o.get('type')} — {o.get('user_id')} — {escape_md(str(o.get('item_name', o.get('amount',''))))}")
        ...
        if action == "view_balances":
            ...
            for uid, info in list(db["users"].items())[:25]:
                s += f"👤 {escape_md(info.get('name','مجهول'))} — ${info.get('balance_usd',0):.2f} (`{uid}`)\n"
        ...
        if action == "users":
            s = "👥 **المستخدمين**\n━━━━━━━━━━━━━━━━━━━━\n"
            for uid, info in list(db["users"].items())[:25]:
                s += f"`{uid}` — {escape_md(info.get('name','مجهول'))}\n"
            await query.edit_message_text(s or "لا يوجد مستخدمين.", parse_mode='Markdown')
            return
        ...
        if action == "tree":
            ...
            for section, roots in db['catalog_roots'].items():
                ...
                def walk(nid, depth):
                    ...
                    lines.append(("  " * depth) + f"`{nid}` {escape_md(node['name'])}{price_txt}{flag}")
        ...
        if action == "search_bot_order":
            ...
            await query.edit_message_text("✍️ اكتب رقم طلب البوت (order_id) أو آيدي الزبون للبحث عنه:", reply_markup=CANCEL_BTN)
            return
        ...
        if action == "toggle_maintenance":
            ...
            await query.edit_message_text(f"وضع الصيانة الآن: {state}\n(عند التفعيل، الزبائن العاديون لا يستطيعون استخدام البوت وتظهر لهم رسالة صيانة).")
            return

        if action == "admin_notes":
            clear_awaiting(ud)
            ud['awaiting_admin_notes'] = True
            current = db.get('admin_notes', '') or 'لا توجد ملاحظات بعد.'
            await query.edit_message_text(f"📝 **الملاحظات الحالية:**\n{escape_md(current)}\n\n✍️ اكتب ملاحظات جديدة لتحديثها:", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return

        if action == "list_admins":
            admins = db.get('authenticated_admins', [])
            lines = ["👮 **الأدمنية المصادقين:**\n━━━━━━━━━━━━━━━━━━━━"]
            for a in admins:
                name = escape_md(db['users'].get(a, {}).get('name', 'غير معروف'))
                lines.append(f"`{a}` — {name}")
            await query.edit_message_text("\n".join(lines) if admins else "لا يوجد أدمنية مصادقين بعد.", parse_mode='Markdown')
            return
        ...
        if action == "export_users":
            ...
            await context.bot.send_document(
                ...
                caption=f"👥 كل المستخدمين ({len(db.get('users', {}))})")
            return
        ...
        if action == "post_channel":
            try:
                ...
                await context.bot.send_message(
                    ADMIN_CHANNEL_ID,
                    "🛸 **لوحة التحكم الإدارية**\nيمكن لأي أدمن مصادق استخدام هذه الأزرار مباشرة من هنا.",
                    reply_markup=admin_panel_markup, parse_mode='Markdown'
                )
                await query.edit_message_text("✅ تم نشر لوحة التحكم في القناة.")
            except Exception as e:
                await query.edit_message_text(f"❌ فشل النشر في القناة: {e}")
            return
        return

    # ---------- الرد على شكوى زبون من القناة ----------
    ...
    if data == "support#start":
        ...
        await query.edit_message_text("📝 اكتب شكواك أو استفسارك الآن بالتفصيل:")
        return
    ...
    if data.startswith("nav#"):
        ...
        if not node:
            ...
            return
        await query.edit_message_text(f"📁 **{escape_md(node['name'])}**", reply_markup=render_listing(db, node['children'], back_cb_for(node), f"nav#{nid}", page), parse_mode='Markdown')
        return
    ...
    if data.startswith("buy#"):
        ...
        if balance < node['price']:
            await query.edit_message_text(f"❌ رصيدك (${balance:.2f}) لا يكفي لشراء [{escape_md(node['name'])}]!")
            return
        ...
        if node['kind'] == 'game_code':
            ...
            await query.edit_message_text(f"🎁 {escape_md(node['name'])}\n✍️ **أدخل الآيدي (ID) الخاص بك:**", parse_mode='Markdown', reply_markup=CANCEL_BTN)
            return

        # بطاقة أو رقم واتساب/تيليجرام: تأكيد قبل الإرسال
        warn = node.get('warning', '')
        btn = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ تأكيد الشراء", callback_data=f"confirm_buy#{nid}")],
            [InlineKeyboardButton("❌ إلغاء", callback_data="cancel_flow")]
        ])
        msg = f"{warn}\n\n🎁 {escape_md(node['name'])}\n💰 السعر: ${node['price']}\n\nهل تريد تأكيد الشراء؟" if warn else f"🎁 {escape_md(node['name'])}\n💰 السعر: ${node['price']}\n\nهل تريد تأكيد الشراء؟"
    ...
    if data.startswith("confirm_game_buy#"):
        ...
        if not node or node.get('deleted') or not node.get('active', True) or not game_id:
            await query.edit_message_text("⚠️ حدث خطأ، حاول الطلب من جديد.")
            return
        balance = get_balance(db, user_id)
        if balance < node['price']:
            await query.edit_message_text(f"❌ رصيدك (${balance:.2f}) لا يكفي!")
    ...
    if data == "confirm_refund":
        ...
        currency = ud.get('refund_currency', 'usd')
        if amount is None or usd_amount is None:
            await query.edit_message_text("⚠️ حدث خطأ، حاول من جديد من قائمة استرجاع الأموال.")
            return
        balance = get_balance(db, user_id)
        if balance < usd_amount:
    ...
    if data == "confirm_phone_order":
        ...
        if amount is None or usd_amount is None or not phone:
            await query.edit_message_text("⚠️ حدث خطأ، حاول من جديد من قائمة شحن الهاتف.")
            return
        balance = get_balance(db, user_id)
        if balance < usd_amount:
            await query.edit_message_text(f"❌ رصيدك (${balance:.2f}) غير كافٍ!")
            return
        order_id = generate_order_id()
        ...
        await query.edit_message_text(f"✅ تم إرسال طلب الشحن (رقم {order_id}) للإدارة.")
        return
    ...
    if data.startswith("order_ok#"):
        ...
        await notify_admin_dm(
            context,
            f"✍️ **اكتب الآن هنا** الكود/الرقم/المعلومات لتسليمها للزبون (طلب {order_id} — {escape_md(order.get('item_name',''))}):"
        )
        return
    ...
    if data.startswith("charge#"):
        ...
        await query.edit_message_text(f"✍️ **أدخل المبلغ** {'بالدولار' if currency=='usd' else 'بالليرة السورية'}:", reply_markup=CANCEL_BTN)
        return
    ...
    if data.startswith("refund#"):
        ...
        await query.edit_message_text(f"✍️ **أدخل المبلغ** {'بالدولار' if currency=='usd' else 'بالليرة السورية'} المراد استرجاعه:", reply_markup=CANCEL_BTN)
        return

    if data == "bot_order#start":
        clear_awaiting(ud)
        ud['awaiting_bot_desc'] = True
        await query.edit_message_text("🤖 **إنشاء بوت جديد**\n✍️ اكتب مواصفات البوت الذي تريده:", reply_markup=CANCEL_BTN)
        return
    ...
    if data.startswith("srv#"):
        ...
        await context.bot.send_message(
            ...
            f"🆔 {user_id}\n💬 {escape_md(contact)}\n📝 {escape_md(desc)}\n🖥️ {srv_name}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("💰 السعر (مع خصم من محفظة الزبون)", callback_data=f"bot_price#{user_id}#{order_id}")],
                [InlineKeyboardButton("⏰ الوقت", callback_data=f"bot_time#{user_id}#{order_id}")],
                [InlineKeyboardButton("📝 تفاصيل/ملاحظات", callback_data=f"bot_notes#{user_id}#{order_id}")],
    ...
    if data.startswith("bot_price#"):
        ...
        await notify_admin_dm(context, f"✍️ اكتب السعر الآن هنا للمستخدم `{parts[1]}` (طلب {parts[2]})، مثال: `10$` أو `10`:")
        return
    ...
    if data.startswith("bot_notes#"):
        ...
        await notify_admin_dm(context, f"📝 اكتب الآن التفاصيل/الملاحظات الخاصة بطلب `{parts[2]}` (تُحفظ ليمكنك الرجوع لها لاحقاً، لا تُرسل للزبون):")
        return
    ...
    if data.startswith("bot_pay#"):
        ...
        if order.get('status') == 'paid':
            await query.edit_message_text("✅ تم دفع هذا الطلب مسبقاً.")
            return
        price = order.get('price')
        if price is None:
            await query.edit_message_text("⚠️ لم يتم تحديد سعر لهذا الطلب بعد.")
            return
        balance = get_balance(db, user_id)
        if balance < price:
            await query.edit_message_text(f"❌ رصيدك (${balance:.2f}) لا يكفي لدفع ${price:.2f}. اشحن محفظتك أولاً.")
            return
        update_balance(db, user_id, -price)
        order['status'] = 'paid'
        db['stats']['purchases'] += 1
        log_activity(db, f"دفع طلب بوت #{order_id} من {user_id} بقيمة ${price:.2f}")
        save_db(db)
        await query.edit_message_text(f"✅ تم خصم ${price:.2f} من محفظتك. سيتم التواصل معك لإكمال طلب البوت 🤖")
        await context.bot.send_message(ADMIN_CHANNEL_ID, f"💰 تم دفع طلب البوت (رقم {order_id}) من قبل الزبون `{user_id}` — تم خصم ${price:.2f}")
        return
